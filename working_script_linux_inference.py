...
def inference(): 
    ...
    process_result = MovingAveragePrediction(
        symbol_mapping=label,
        pred_hist_length=3,
        pred_conf_thresh=0.3
    )
    ...
    while True: 
        ...
        # print(output_array)
        data_df = recall_data(output_array)
        data_df_short = data_df.iloc[:, 3:6].values
        # print(data_df_short)
        data_empty = np.zeros((99,))
        data_concate = np.concatenate((data_df_short.reshape(-1,1), data_empty.reshape(-1,1)))
        data_result = data_concate.reshape(-1,1)
        data_result = data_result[:data_empty.shape[0]]
        # input_data_queue.put(output_array)
        # print("--------input data queue----------------")
        input_data_queue.put(data_result)
        # print(input_data_queue)

        if input_data_queue.qsize() > 0:
            # print("before: the size is ......",input_data_queue.qsize())
            input_data = input_data_queue.get()
            # prediction = model.predict(input_data)[0]
            # print("------------prediction tensorflow---------------")
            prediction = model.predict(input_data.reshape(1,-1))
            return_queue.put(prediction)
            process_result.add_prediction(prediction[0])
            # print(prediction)
            # print("after: the size is ......",input_data_queue.qsize())
            process_result.print_result()
            # time.sleep(0.1)

        if input_data_queue.qsize() > 5: 
            print("Clear input data queue")
            print(prediction)
            while not input_data_queue.empty(): 
                input_data_queue.get()
            # stop_TI_radar()
            # break 
...
class MovingAveragePrediction(): 
    def __init__(self, symbol_mapping, pred_hist_length: int=1, pred_conf_thresh: float=0.0):
        self._history_length = pred_hist_length
        self._confidence_threshold = pred_conf_thresh
        self._prediction_history = collections.deque(maxlen=self._history_length)
    ...
    def print_result(self): 
        history_np = np.array(self._prediction_history)
        history_np_mean = history_np.mean(axis=0)
        max_index = np.argmax(history_np_mean)
        max_mean_confidence = history_np_mean[max_index]
        # print(history_np)
        if max_index !=0 and max_mean_confidence > self._confidence_threshold: 
            class_name = self._mapping[max_index]
            print(f"{class_name} - mean confidence: {max_mean_confidence:1.3f}")
